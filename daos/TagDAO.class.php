...
class TagDAO {
    public static function find_all_tags_in_discipline($discipline_id){
      $tags = array();
      if (!is_null($discipline_id)) {
          $query = "SELECT * FROM `tag` WHERE discipline_id = " .$discipline_id .";";
    ...
    public static function getUserTags($user_id) {
        $tags = array();
        if (!is_null($user_id)) {
            $query = "SELECT tag.tag_id, tag_name, discipline_id, clicks FROM user_tag NATURAL JOIN tag WHERE user_tag.user_id =" .$user_id .";";
            $result = PDOAccess::returnSQLquery($query);
